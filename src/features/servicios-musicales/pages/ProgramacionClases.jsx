...
const ProgramacionClases = () => {
  ...
  const getTitle = () => {
    ...
    if (isCliente) {
      return "Clases de mis beneficiarios";
    }
  ...
  // Resolver robustamente el ID de referencia del cliente para filtrar clases por beneficiarios asociados
  const [clienteRefId, setClienteRefId] = useState(null);

  // Utilidad: dejar solo dígitos
  const onlyDigits = (s) => (s || '').toString().replace(/\D/g, '');

  // Resolver clienteRefId del usuario autenticado (cliente) utilizando beneficiarios y usuarios_has_rol
  const resolveClienteIdForUser = async (user) => {
    try {
      ...
      const beneficiariosData = Array.isArray(benefRes.data?.beneficiarios)
      ...
      let clienteBeneficiario = beneficiariosData.find(b => userUhrs.some(u => String(u?._id) === String(b?.usuario_has_rolId)));

      // 2) Fallback por documento (solo dígitos)
      if (!clienteBeneficiario && user?.documento) {
        const userDoc = onlyDigits(user.documento);
        clienteBeneficiario = beneficiariosData.find(b => onlyDigits(b?.numero_de_documento || b?.numeroDocumento) === userDoc);
      }
      ...
      if (!clienteBeneficiario && user?.email) {
        ...
        if (uhrByEmail) {
          clienteBeneficiario = beneficiariosData.find(b => b?.usuario_has_rolId === uhrByEmail?._id);
        }
        if (!clienteBeneficiario) {
          clienteBeneficiario = beneficiariosData.find(b => ((b?.email || b?.correo || '').toLowerCase() === email));
        }
      }
      ...
      const clientRef = esClientePropio || !clienteBeneficiario?.clienteId
        ? clienteBeneficiario?._id
        : clienteBeneficiario?.clienteId;

      return clientRef || null;
    } catch (err) {
      console.warn('resolveClienteIdForUser error:', err);
      return null;
    }
  };

  // Efecto para resolver el clienteRefId cuando el usuario sea cliente
  useEffect(() => {
    let mounted = true;
    const run = async () => {
      if (!isCliente) {
        if (mounted) setClienteRefId(null);
        return;
      }
      const ref = await resolveClienteIdForUser(user);
      if (mounted) setClienteRefId(ref);
    };
    run();
  ...
  const clasesProcessed = useMemo(() => {
    ...
    return clases
      .filter((clase) => {
        ...
        // Si es cliente, filtrar clases por beneficiarios asociados al clienteRefId usando ventas (populate=true)
        if (isCliente && pasaFiltro) {
          const clienteId = clienteRefId;
          if (!clienteId) {
            console.log("❌ Clase filtrada para cliente (sin clienteRefId resuelto)", { id: clase._id });
            return false;
          }
          const clienteIdStr = String(clienteId);
          // Construir un set de IDs de beneficiarios que pertenecen a este cliente, usando ventas con populate=true
          const benefIdsForClient = new Set(
             (beneficiarios || [])
               .filter(v => {
                 const b = v?.beneficiarioId;
                 const ventaClienteId = String(v?.clienteId || '');
                 const bClienteId = String(b?.clienteId || '');
                 return (b && bClienteId === clienteIdStr) || (ventaClienteId && ventaClienteId === clienteIdStr);
               })
               .map(v => String(v?.beneficiarioId?._id || v?.beneficiarioId || ''))
           );
          if (benefIdsForClient.size === 0) {
            console.log("ℹ️ Cliente sin beneficiarios asociados (por ventas)", { clienteId: clienteIdStr });
          } else {
            console.log("👨‍👩‍👧 Beneficiarios del cliente para filtrado:", Array.from(benefIdsForClient));
          }

          // Verificar si alguno de los beneficiarios de la clase pertenece al cliente
          const asociadoPorSet = clase.beneficiarios?.some(venta => {
            const bId = String(venta?.beneficiarioId?._id || venta?.beneficiarioId || '');
            return benefIdsForClient.has(bId);
          });

          // Fallback adicional: si el cliente es su propio beneficiario, permitir coincidencia directa por ID
          const asociadoDirecto = clase.beneficiarios?.some(venta => {
            const bId = String(venta?.beneficiarioId?._id || '');
            return bId && bId === clienteIdStr;
          });

          const asociado = asociadoPorSet || asociadoDirecto;

          if (!asociado) {
            console.log("❌ Clase filtrada para cliente (no asociado)", {
              id: clase._id,
              clienteRefId: clienteIdStr,
              benefIdsForClient: Array.from(benefIdsForClient),
            });
            return false;
          }
        }
  ...
  }, [clases, beneficiarios, user, clienteRefId]);
                  ...
                  return (
                    ...
                    >
                      {tieneClase && clasePrincipal ? (
                        ...
                        !isBeneficiario && !isCliente && canCreateClase && (
                          <Box
                            sx={{
                              height: "100%",
                              display: "flex",
